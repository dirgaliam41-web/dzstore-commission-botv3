...
if (!need("TICKET_CATEGORY_ID")) process.exit(1);
if (!need("FEEDBACK_CHANNEL_ID")) process.exit(1);
...
// Migrasi aman untuk database lama: versi sebelumnya mengizinkan beberapa
// ticket completed sebelum feedback. Pertahankan ticket terbaru sebagai ticket
// aktif dan arsipkan duplikat lama agar unique index dapat dibuat.
db.exec(`
  WITH ranked AS (
    SELECT id,
           ROW_NUMBER() OVER (PARTITION BY guild_id, user_id ORDER BY id DESC) AS rn
    FROM tickets
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS one_active_ticket_per_user
...
const commissionLabel = t =>
  t.size ? `${t.category.toUpperCase()} ${t.size}` : t.category.toUpperCase();
  ...
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        {
          label: "SKIN",
          description: "Minecraft Skin 64 / 128 / 512",
          value: "skin",
          emoji: "🧑‍🎨"
        },
        {
...
const commands = [
  new SlashCommandBuilder()
    ...
    .setDescription("Pasang panel order commission."),
  new SlashCommandBuilder()
...
async function openOrder(interaction, category, size) {
  const modal = new ModalBuilder()
    ...
    .setTitle(`Order ${size ? `SKIN ${size}` : category.toUpperCase()}`);
...
async function createTicket(interaction, category, size) {
  // completed masih aktif karena customer belum mengirim feedback.
  const active = db.prepare(
    "SELECT * FROM tickets WHERE guild_id=? AND user_id=? AND status!='archived' ORDER BY id DESC LIMIT 1"
  ).get(interaction.guildId, interaction.user.id);

  if (active) {
    return interaction.reply({
      content: `❌ Kamu masih punya ticket aktif: <#${active.channel_id}> (${active.order_id}).\n\nSelesaikan ticket tersebut dan kirim feedback terlebih dahulu sebelum membuat order baru.`,
      ephemeral: true
    });
  }
  ...
  try {
    ...
    // Pengaman tambahan untuk dua klik/order bersamaan.
    await channel.delete("Duplicate active DZS commission ticket").catch(() => {});
    ...
    if (activeNow) {
      return interaction.reply({
        content: `❌ Kamu masih punya ticket aktif: <#${activeNow.channel_id}> (${activeNow.order_id}).`,
        ephemeral: true
      });
    }
  ...
  if (log && log.isTextBased()) {
    await log.send(
      `🎫 Order baru **${oid}** — ${categoryLabel(t)} — <@${interaction.user.id}>`
    ).catch(() => {});
  }
...
async function closeTicket(interaction) {
  ...
  await interaction.reply({
    content: `✅ ${updated.order_id} selesai. Silakan isi feedback. Ticket akan otomatis dihapus setelah feedback dikirim.`,
    ephemeral: true
  });
}
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === "setup-commission") {
        return saveOrUpdateCommissionPanel(interaction);
      }
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith("dzs_review_")) {
        ...
        const feedbackChannel =
          interaction.guild.channels.cache.get(process.env.FEEDBACK_CHANNEL_ID);
        ...
        await interaction.reply({
          content: `✅ Feedback **${oid}** berhasil disimpan! 💜 Ticket akan dihapus sekarang.`,
          ephemeral: true
        });

        // Hapus setelah interaction berhasil dibalas agar customer tetap menerima konfirmasi.
        await interaction.channel.delete("DZS Commission feedback submitted").catch(() => {});
      }
    }
  } catch (err) {
